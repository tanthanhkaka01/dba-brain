...
| `interval_rates.py` | Pulling a collector's `key=value` fields back out of the message that carried them. Shared so the report and the alert split the same sample the same way. It also held the two-sample rate arithmetic until 2026-08-11, when `PERFORMANCE_IO_LATENCY`'s interval grading was withdrawn — see `docs/04_metrics_engine.md`. | `message_fields` |
