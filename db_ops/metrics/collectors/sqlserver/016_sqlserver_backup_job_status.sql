    ...
    CONCAT(
        ...
        ', last_status=',
            CASE run_status
                ...
                ELSE 'NO_HISTORY'
            END,
        ', last_run_datetime=', COALESCE(CONVERT(varchar(19), run_datetime, 120), 'NULL')
    ) AS message
FROM latest_run
WHERE rn = 1
...
-- -- check next if have critical
-- DECLARE @JobNamePattern nvarchar(200) = N'%Job_Maintain_Backup%';
...
-- FROM h
-- WHERE run_status = 0
-- ORDER BY run_datetime DESC, step_id;