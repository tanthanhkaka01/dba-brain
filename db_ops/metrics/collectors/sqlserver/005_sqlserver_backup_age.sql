...
SELECT
    ...
    CASE
        WHEN lb.last_backup_finish_date IS NULL THEN
            CONCAT('No full or differential backup found for database ', d.name COLLATE DATABASE_DEFAULT, '.')
        ELSE
            CONCAT(
                'Last full/differential backup ',
                DATEDIFF(hour, lb.last_backup_finish_date, GETDATE()),
                ' hours ago for database ',
