...
SELECT
    ...
    CONCAT(
        ...
        ', top_session_id=', MAX(CASE WHEN rn = 1 THEN session_id END),
        ', top_status=', MAX(CASE WHEN rn = 1 THEN request_status END),
        ', command=', MAX(CASE WHEN rn = 1 THEN command END),
        ', wait_type=', COALESCE(MAX(CASE WHEN rn = 1 THEN wait_type END), ''),
        ', blocking_session_id=', COALESCE(CAST(MAX(CASE WHEN rn = 1 THEN blocking_session_id END) AS varchar(32)), ''),
        ', cpu_seconds=', MAX(CASE WHEN rn = 1 THEN cpu_seconds END),
        ', reads=', MAX(CASE WHEN rn = 1 THEN reads END),
        ', writes=', MAX(CASE WHEN rn = 1 THEN writes END),
        ', logical_reads=', MAX(CASE WHEN rn = 1 THEN logical_reads END),
        ', login=', COALESCE(MAX(CASE WHEN rn = 1 THEN login_name END), ''),
        ', host=', COALESCE(MAX(CASE WHEN rn = 1 THEN host_name END), ''),
        ', app=', COALESCE(MAX(CASE WHEN rn = 1 THEN program_name END), '')
    ) AS message
FROM ranked
