...
(
    SELECT
        ...
        failed_runs_24h,
        CAST(
            failed_runs_24h * 100.0
            / NULLIF(total_runs_24h, 0)
            AS decimal(10, 2)
...
SELECT
    ...
    CASE
        WHEN fail_rate_percent >= 100 THEN 'CRITICAL'
    ...
    CONCAT(
        ...
        ', last_failed_datetime=',
        COALESCE(
            CONVERT(varchar(19), last_failed_datetime, 120),
            'NULL'
        )
    ) AS message
FROM failed_jobs
