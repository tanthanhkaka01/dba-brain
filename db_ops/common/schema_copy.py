...
def assert_destination(cursor: Any, *, database: str, instance: str = "") -> dict[str, Any]:
    ...
    **``DB_NAME()`` alone is not an identity.** One estate had ``REDACTEDDB_Prod`` on both the UAT and
    ...
    may name the instance (``REDACTEDDB-DB\\REDACTEDDB``) without knowing the host's full computed name.
    """
...
def copy_schema(request: Any, *, progress: Callable[[str], None] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        _close(source_connection)
        if dest_connection is not None:
            _close(dest_connection)
