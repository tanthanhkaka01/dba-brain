...
def test_the_wrong_database_is_refused() -> None:
    with pytest.raises(schema_copy.SchemaCopyError, match="expected 'REDACTEDDB_Prod'"):
        schema_copy.assert_destination(IdentityCursor("SRV\\A", "REDACTEDDB_Testing"),
                                       database="REDACTEDDB_Prod")


def test_the_right_database_on_the_wrong_instance_is_refused() -> None:
    """One estate had `REDACTEDDB_Prod` on both the UAT and the production instance."""
    with pytest.raises(schema_copy.SchemaCopyError, match="does not match"):
        schema_copy.assert_destination(IdentityCursor("UAT-DB\\REDACTEDDB", "REDACTEDDB_Prod"),
                                       database="REDACTEDDB_Prod", instance="REDACTEDDB-DB\\REDACTEDDB")


def test_the_right_instance_passes_and_no_assertion_is_warned_about() -> None:
    ok = schema_copy.assert_destination(IdentityCursor("REDACTEDDB-DB\\REDACTEDDB", "REDACTEDDB_Prod"),
                                        database="REDACTEDDB_Prod", instance="REDACTEDDB-DB\\REDACTEDDB")
    assert "warning" not in ok
    loose = schema_copy.assert_destination(IdentityCursor("ANY\\THING", "REDACTEDDB_Prod"),
                                           database="REDACTEDDB_Prod")
