...
def test_the_wrong_database_is_refused() -> None:
    with pytest.raises(schema_copy.SchemaCopyError, match="expected 'HRMS_Prod'"):
        schema_copy.assert_destination(IdentityCursor("SRV\\A", "HRMS_Testing"),
                                       database="HRMS_Prod")


def test_the_right_database_on_the_wrong_instance_is_refused() -> None:
    """One estate had `HRMS_Prod` on both the UAT and the production instance."""
    with pytest.raises(schema_copy.SchemaCopyError, match="does not match"):
        schema_copy.assert_destination(IdentityCursor("UAT-DB\\HRMS", "HRMS_Prod"),
                                       database="HRMS_Prod", instance="HRMS-DB\\HRMS")


def test_the_right_instance_passes_and_no_assertion_is_warned_about() -> None:
    ok = schema_copy.assert_destination(IdentityCursor("HRMS-DB\\HRMS", "HRMS_Prod"),
                                        database="HRMS_Prod", instance="HRMS-DB\\HRMS")
    assert "warning" not in ok
    loose = schema_copy.assert_destination(IdentityCursor("ANY\\THING", "HRMS_Prod"),
                                           database="HRMS_Prod")
